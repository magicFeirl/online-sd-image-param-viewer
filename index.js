...
const paramsToHTMLStr = (params) => {
  ...
  return result
    .map(
      (s) =>
        `<span class="tag ${highlight_keys.some((k) => s.includes(k)) ? "highlight" : ""
        }">${s.replace(/\\n/g, "<br/>")}</span>`
    )
    .join("\n");
};
...
const parseParams = (tags) => {
  const p = tags["parameters"] || tags["UserComment"];
  let value = p.value
  ...
  div.innerHTML = `
      ...
      .map(
        (title) => `<div>
          ...
          <div class="content">${title === "parameters"
...
document.addEventListener('paste', (event) => {
  ...
  if (!items) {
    return alert('Can\' access clipboard')
  }
